...
fn test_io_enums_serialization() {
    ...
    assert_eq!(serde_json::to_string(&OutputType::PushPull5v).unwrap(), "\"push-pull5v\"");
    assert_eq!(serde_json::to_string(&OutputType::PushPull3v).unwrap(), "\"push-pull3v\"");
    assert_eq!(serde_json::to_string(&OutputType::OpenDrain).unwrap(), "\"open-drain\"");
    
    // Test PullType serialization
    assert_eq!(serde_json::to_string(&PullType::PullUp5v).unwrap(), "\"pull-up5v\"");
    assert_eq!(serde_json::to_string(&PullType::PullUp3v).unwrap(), "\"pull-up3v\"");
    assert_eq!(serde_json::to_string(&PullType::PullDown).unwrap(), "\"pull-down\"");
    assert_eq!(serde_json::to_string(&PullType::Float).unwrap(), "\"float\"");
}
...
fn test_display_types() {
    ...
    assert_eq!(serde_json::to_string(&QrCorrectionType::Low).unwrap(), "\"L\"");
    assert_eq!(serde_json::to_string(&QrCorrectionType::Medium).unwrap(), "\"M\"");
    assert_eq!(serde_json::to_string(&QrCorrectionType::Quality).unwrap(), "\"Q\"");
    assert_eq!(serde_json::to_string(&QrCorrectionType::High).unwrap(), "\"H\"");
    
    // Test DisplayRawColorDepth
    assert_eq!(serde_json::to_string(&DisplayRawColorDepth::OneBit).unwrap(), "\"1\"");
    assert_eq!(serde_json::to_string(&DisplayRawColorDepth::FourBit).unwrap(), "\"4\"");
    assert_eq!(serde_json::to_string(&DisplayRawColorDepth::SixteenBit).unwrap(), "\"16\"");
...
fn test_error_types() {
    ...
    assert_eq!(format!("{}", pin_error), "Invalid pin number: 15. Valid range is 0-11");
    
    let connection_error = ObnizError::Connection("Failed to connect".to_string());
    assert_eq!(format!("{}", connection_error), "Connection error: Failed to connect");
