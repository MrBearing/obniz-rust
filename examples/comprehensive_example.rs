...
async fn main() -> ObnizResult<()> {
    ...
    println!("✓ AD0 voltage: {:.2}V ({}%)", voltage, AdManager::voltage_to_percentage(voltage));
    ...
    pwm.configure_channel(2, PwmConfig {
        io_pin: 6,
        frequency: 1000,
        pulse_width_ms: 0.0,
    }).await?;
    ...
    for i in 1..=5 {
        ...
        println!("Reading {}: ", i);
        for reading in sensor_readings.iter().take(4) { // Show first 4 channels
            let percentage = AdManager::voltage_to_percentage(reading.voltage);
            println!("  Sensor AD{}: {:.2}V ({:.1}%)", reading.channel, reading.voltage, percentage);
        }
    ...
    sleep(Duration::from_secs(2)).await;
    display.qr("https://github.com/your-username/obniz-rust", QrCorrectionType::Medium).await?;
